...
4. **Handler entry.** `module.exports` in `azure-functions/m2m-query/index.js:1120`.
   An `OPTIONS` request returns CORS headers and stops (`index.js:1122`). A `requestId`
   is generated for log correlation (`index.js:1140`).

5. **Body is destructured and the mode chosen.** `index.js:1180`. `isBuilderMode` is true
   only when `mode === 'builder'` (`index.js:1190`); `isRawMode` is true only when a
   non-empty `rawSql` string was posted (`index.js:1197`). Our example is neither, so it
   takes the plain chat path.

6. **Database and prompt are selected.** `index.js:1219-1239`. `database === 'm2mdata99'`
...
7. **Glossary is prepended to the schema.** `buildSchemaWithGlossary` at
   `index.js:233`, called at `index.js:1244`. `activeSchema` now starts with a
   `<mac_glossary>` block.

8. **Conversation is assembled and scrubbed.** `index.js:1283-1293` copies history into
   `claudeMessages`, running every user turn through `scrubPII` (`index.js:455`).
   `userMessageText` is set at `index.js:1299` — for chat mode, the scrubbed question.

9. **Cache is checked.** `index.js:1318-1320`. `cacheable` requires chat mode, a
   `message`, and **no** history. On a first-turn question `cached` may be a hit from
   `getCachedResult` (`index.js:146`), keyed `(question, database)` with a 1-hour TTL.
   A hit skips steps 10 and 11 but still executes the SQL, so rows are always fresh.

10. **Clarifier runs.** `index.js:1331` calls `runClarifier` (`index.js:578`), a Haiku call
    with a 4-second timeout. It returns either `{ok:false, question}` — in which case
    `index.js:1344` returns the question to the user and stops — or
    `{ok:true, constraints}`. `constraints` holds `{status: 'Open'}` for our example, and
    is stored in the handler variable at `index.js:1332`.

11. **Claude generates SQL.** `index.js:1376` calls `callClaudeAPI` (`index.js:642`) with
    the static instructions and schema as two cached system blocks.
    `parseClaudeResponse` (`index.js:690`) pulls `{explanation, sqlQuery}` out of the JSON.
    `sqlQuery` now holds something like
    `SELECT ... FROM SOMAST WHERE RTRIM(FPARTNO) LIKE '%41B531222G2%'`.

12. **SQL is cleaned.** `cleanSqlQuery` at `index.js:779`, called at `index.js:1383`.
    Strips comments and pulls the `SELECT` out if the model wrapped it in prose. If
    `sqlQuery` is empty the handler returns the explanation alone (`index.js:1386`).

13. **Safety check.** `validateSqlSafety` (`index.js:798`) at `index.js:1396`. Rejects
    non-SELECT statements, restricted tables and columns from `shared/restricted.js`,
    write keywords, and `SELECT *`.

14. **Schema check.** `validateAgainstSchema` (`index.js:859`) at `index.js:1433`, using
...
15. **Constraint check.** `validateAgainstConstraints` (`index.js:900`) at
    `index.js:1478`. Compares the SQL against the `constraints` from step 10. For our
    example it rejects `NOT IN ('Closed','Cancelled')` because that also admits `On Hold`
    and `Revised`, and sends it back for one repair.

16. **Pooled connection.** `getDbPool` (`index.js:104`) at `index.js:1513`. Pools are keyed
    by connection string and reused across invocations.

17. **Execute.** `index.js:1531`, inside a loop that allows `MAX_RETRIES = 2` (three
    attempts total). On success, `result.recordset` holds the rows.

18. **Zero-row retry.** `index.js:1620`. If the recordset is empty and this is not raw
    mode, the model is asked once to broaden the query.

19. **Semantic critic.** `semanticSanityCheck` (`index.js:932`) at `index.js:1670`. Runs
    only when there is at least one row. Appends a warning sentence to the explanation.
    Advisory — never blocks.

20. **Cache write.** `setCachedResult` (`index.js:157`) at `index.js:1682`, gated on
    `cacheable` and a non-empty result.

21. **Response.** `index.js:1690`. Body carries `explanation`, `sql`, `columns`,
    `columnSources` (from `parseColumnSources`, `index.js:737`), `rows`, `rowCount`,
    `_requestId`, and `_cost`.

22. **Cost is recorded.** The `finally` block at `index.js:1709` fires an un-awaited
    async insert into `query_costs` on `CHAT_DB_CONNECTION`.

23. **Frontend renders.** `src/App.tsx:425-472` turns the response into an assistant
...
`composeBuilderMessage` (`index.js:471`) replaces the free-text question with an XML
constraint block. Skips the clarifier and the cache (`index.js:1319`, `index.js:1331`).
...
(`index.js:1398`, `index.js:1433`), and the zero-row retry is skipped (`index.js:1620`).

**Cache hit** — `index.js:1372` reuses the stored explanation and SQL. Clarifier and
...
|---|---|---|---|---|
| 1 | Anthropic HTTP 429/500/503/529 (`index.js:678`) | Nothing — plain resend | 3 retries, delays 1s/2s/4s (`index.js:646`) | Throws; caught at `index.js:1703`, returns 500 |
| 2 | Safety check failed (`index.js:1397`) | "Your query was rejected: ... You MUST only generate SELECT queries" (`index.js:1409`) | 1 repair | 400 with `safety.reason` (`index.js:1421`) |
| 3 | Schema check failed (`index.js:1434`) | The list of bad tables/columns plus "You MUST only use table and column names from the schema" (`index.js:1447`) | 1 repair, then revalidated | 400 with the remaining errors (`index.js:1460`) |
| 3b | Constraint check failed (`index.js:1478`) | The reason, plus "Filter to exactly that status ... Do not use NOT IN" (`index.js:1482`) | 1 repair, then re-checked | 400 with the reason (`index.js:1499`) |
| 4 | SQL error at execution (`index.js:1534`) | The database error text, the failed query, and targeted advice from `getErrorGuidance` (`index.js:1106`) | `MAX_RETRIES = 2`, so 3 executions | `lastError` rethrown at `index.js:1608`, becomes a 500 |
| 5 | Connection dropped: `ECONNCLOSED`/`ENOTOPEN`/`ESOCKET` (`index.js:1537`) | Nothing — pool is deleted and rebuilt | Inside the same 3-attempt loop | Falls through to path 4 |
| 6 | Zero rows returned (`index.js:1620`) | "returned ZERO rows ... regenerate the query with BROADER filters" (`index.js:1623`) | 1 attempt, best-effort | Original zero-row result stands (`index.js:1630`) |
| 7 | Pool creation threw (`index.js:1514`) | n/a | Deletes the cached pool, tries once more | Throws to the outer catch |
| 8 | Clarifier failed or timed out (`index.js:625`) | n/a | None | Returns `{ok:true}` — fails open, query proceeds without constraints |
| 9 | Cost insert failed (`index.js:1749`) | n/a | None | Logged only. Cost still rides back on the response |
| 10 | Semantic mismatch detected (`index.js:1672`) | n/a | None | A note is appended to the explanation. Never blocks |

Paths 2, 3, 3b, 4, and 6 all run through one shared helper: `repair`, defined at
`index.js:1265`. It appends the assistant's last output and the reason text to a message
list, calls Claude, and returns `{explanation, sql, messages}`.

Every repair round trip is counted by `accrueUsage` (`index.js:1163`), which increments
...
|---|---|---|
| Anthropic API | `callClaudeAPI` (`index.js:642`), `runClarifier` (`index.js:578`) | `@anthropic-ai/sdk`, client built in `getAnthropicClient` (`index.js:632`) |
...
| UniPoint Quality | `m2m-query` only (`index.js:1226`) | `mssql`, pooled |
...
| Variable | Read by |
|---|---|
| `ANTHROPIC_API_KEY` | `m2m-query/index.js:632`, checked at `index.js:1209` |
...
| `M2M_IMPULSE_CONNECTION_STRING` | `m2m-query/index.js:1220` |
| `UNIPOINT_CONNECTION_STRING` | `m2m-query/index.js:1226` |
| `CHAT_DB_CONNECTION` | `chat-sessions`, `chat-messages`, `query-costs`, `activity-report`, `m2m-query/index.js:1716` |
...
**The cost-recording pool has a deliberately short timeout.** `index.js:65-71`:
...
**The query cache refuses any turn with history.** `index.js:1307-1313`:
...
**Some `let`s are declared before the `try` on purpose.** `index.js:1154-1156`:
...
**Dead code, currently.**
- `checkCostCap` (`index.js:176`) is never called, so `DAILY_USER_COST_LIMIT` and
  `DAILY_GLOBAL_COST_LIMIT` (`index.js:173-174`) have no effect.
- `getCostPoolSafe` (`index.js:85`) is never called, and `getCostPool` (`index.js:49`) is
  only called by it — so the whole cost-pool chain is unreachable. The cost insert in the
  `finally` block builds its own `sql.ConnectionPool` instead (`index.js:1728`).
...
`_internals` (`index.js:1767`) also does not export `parseClaudeResponse`, so there is no
current replacement to point the test at.

**The zero-row retry still tells the model to drop status filters.** `index.js:1626`:
"don't filter by status unless the user specifically asked for a status", and
`index.js:1628`: "remove unnecessary status filters." This is now only reachable when the
user named no status — `index.js:1620` gates on `!(constraints && constraints.status)` — so
it can no longer override an explicit request, but the wording still misleads a reader.

**The semantic critic's status branch cannot fire, and is now redundant.** `index.js:946-952`. The outer `if`
...
silently. The comment describes the opposite behavior. `validateAgainstConstraints` now
catches this case before execution, so the branch is dead weight rather than a real gap.

**`validateAgainstConstraints` scans the whole SQL for `STATUS`.** `index.js:906` tests
...
**A preset filters on a status value that does not exist.** `src/App.tsx:989` uses
`WHERE fstatus = 'O'`, but FSTATUS holds whole words. Confirmed 2026-09-08 by running the
preset against the live endpoint: it returns `MissingStreetAddress: 0`. It has been
silently answering zero; it means `fstatus = 'Open'`.

**Connection-string parsing is copy-pasted eight times.** The same ~8-line split loop
appears at `m2m-query/index.js:53`, `:108`, `:1722`, `chat-sessions/index.js:17`,
...
`getDbPool` (`index.js:104`) doing the same job.
...
**ERP pools disable encryption; app pools require it.** `index.js:120` sets
`{ encrypt: false, trustServerCertificate: true }` for the ERP and UniPoint connections,
while `index.js:64` and the chat functions set `{ encrypt: true }`.

**CORS is fully open.** Every function returns `'Access-Control-Allow-Origin': '*'`, for
example `index.js:1135`. Access control rests on the `?code=` function key and the
...
accepts whatever `database` value is posted (`index.js:1215`) without checking the caller.
...
**Gemini naming survives the removal.** The variable `geminiCalls` (`index.js:1153`) and
the column `gemini_calls` (`schema.sql`) both now count Claude calls. `index.js:135` still
